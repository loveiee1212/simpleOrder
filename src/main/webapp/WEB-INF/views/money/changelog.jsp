...
<body>
	<div id="calendar">
		...
		<button onclick="location.href='changevc'">나가기</button>
	</div>
						...
						for (var j = firstDay.getDay(); j < firstDay.getDay() + lastDay.getDate(); j++) {
							...
							if (bd_dateYM == $("#yearMonth").text()) {
								if (bd_date == $tdDay.eq(j).text()) {
									if (data[i].VC_CODE == "1") {
										$tdText.eq(j).append($("<span>").html("시작시재 : "+start+"\r\n"));
									}
									if(data[i].VC_CODE == "-1"){
										$tdText.eq(j).append($("<span>").html("마감시재 : "+end+"\r\n"));
									}
								}
							}
						}
...
function getDayVCLog(day) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			$table.append($("<tr><th>시간</th><th>구분</th><th>금액</th><th>차액</th><th>사유</th></tr>"));
			for ( var i in data) {
				$tr = $table.append($("<tr>"));
				$tr.append($("<td>").html(moment(data[i].VC_CHANGETIME).format("HH:mm")));
				if(data[i].VC_CODE == "1"){
				...
				if(data[i].VC_CODE != "1"){
					$tr.append($("<td>").html(data[i].VC-data[i-1].VC));
				}
